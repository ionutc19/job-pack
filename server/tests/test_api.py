    ...
    response = client.post(
        ...
        json={
            "cv_text": (
                ...
                "with django and flask expertise"
            ),
            "job_description": (
                "Looking for a python developer "
                "with react and docker experience"
            ),
            "language": "en",
        },
        headers={"X-User-Id": "test-jf-en"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_job_fit_analyze_ro():
    response = client.post(
        ...
        json={
            "cv_text": (
                "Dezvoltator python experimentat "
                "cu experiență în django și flask"
            ),
            "job_description": (
                "Căutăm un dezvoltator python "
                "cu experiență în react și docker"
            ),
            "language": "ro",
        },
        headers={"X-User-Id": "test-jf-ro"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_job_fit_default_language():
    response = client.post(
        ...
        headers={"X-User-Id": "test-jf-default"},
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.post(
        ...
        json={
            "cv_text": (
                ...
                "experience in django and rest APIs"
            ),
            "job_description": (
...
def test_apply_letter_ro():
    response = client.post(
        ...
        json={
            "cv_text": (
                "Dezvoltator Python cu 5 ani "
                "experiență în django și REST APIs"
            ),
            "job_description": (
                "Avem nevoie de un inginer backend "
                "python cu experiență django"
            ),
            "tone": "professional",
            "language": "ro",
        },
        headers={"X-User-Id": "test-al-ro"},
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.post(
        ...
        json={
            ...
            "description": (
                ...
                "the job fit screen on Android 14."
            ),
        },
    )
    assert response.status_code == 200
...
def test_usage_meta_in_response():
    response = client.post(
        "/api/job-fit/analyze",
        json={
            "cv_text": (
                "Experienced python developer "
                "with django expertise"
            ),
            "job_description": (
                "Looking for a python developer "
        ...
        headers={"X-User-Id": "test-usage-meta"},
    )
    assert response.status_code == 200
    data = response.json()
    ...
    assert "show_upgrade" in usage
    assert "period_seconds" in usage


def test_entitlements_me():
    response = client.get(
        "/api/entitlements/me",
        headers={"X-User-Id": "test-ent-me"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_admin_set_tier_requires_secret():
    response = client.post(
        "/api/admin/set-tier",
        json={"tier": "premium"},
        headers={"X-User-Id": "test-admin-noauth"},
    )
    assert response.status_code == 403


def test_free_tier_limit():
    device = "test-free-limit-v2"
    for _i in range(5):
        response = client.post(
            "/api/job-fit/analyze",
            json={
...
def test_verify_purchase_invalid_product():
    response = client.post(
        "/api/entitlements/verify-purchase",
        json={
            "product_id": "invalid_product",
            "purchase_token": "tok123",
        },
        headers={"X-User-Id": "test-purchase"},
    )
    assert response.status_code == 422


def test_verify_purchase_no_credentials():
    response = client.post(
        "/api/entitlements/verify-purchase",
        json={
            "product_id": "jobcoach_premium_monthly",
            "purchase_token": "tok_test_123",
        },
        headers={"X-User-Id": "test-purchase-nocred"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_rtdn_endpoint_exists():
    response = client.post(
        "/api/admin/rtdn",
        json={"message": {"data": ""}, "subscription": ""},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_user_identity_stable():
    uid = "test-stable-uid-abc"
    client.post(
        "/api/job-fit/analyze",
        json={
            "cv_text": (
    ...
    data = response.json()
    assert data["tier"] == "free"
    assert data["usage"]["job_fit"]["used"] == 1
